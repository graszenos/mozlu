...
class QuiddichAPIClient {
    ...
    async countdown(seconds) {
        for (let i = seconds; i >= 0; i--) {
            ...
            process.stdout.write(`[${timestamp}] [*] Enteni ${i} detik kanggo nerusake...`);
    ...
    async checkProxyIP(proxy) {
        try {
            ...
            if (response.status === 200) {
                ...
                throw new Error(`Ora bisa verifikasi IP saka proxy. Status code: ${response.status}`);
            }
        } catch (error) {
            throw new Error(`Kesalahan verifikasi IP proxy: ${error.message}`);
        }
    }
    ...
    async makeRequest(url, method, headers, proxy) {
        ...
        try {
            ...
            this.log(`Kesalahan nggawe permintaan: ${error.message}`, 'error');
            return null;
        }
    }
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                try {
                    ...
                    this.log(`Kesalahan verifikasi IP proxy: ${error.message}`, 'error');
                    continue;
                }
                ...
                if (!userInfo.is_exist) {
                    ...
                    if (giftResult && giftResult.describe === "Sampeyan entuk hadiah golden snitch kanthi sukses.") {
                        this.log("Sampeyan entuk hadiah golden snitch kanthi sukses", 'success');
                    }
                ...
                if (userInfo && userInfo.is_exist) {
                    ...
                    console.log(`========== Akun ${i + 1} | ${userData.username.green} | ip: ${proxyIP} ==========`);
                    ...
                    if (collectInfo) {
                        ...
                        if (collectInfo.usdt > 0) {
                            ...
                            if (usdtResult && usdtResult.describe === "Ngoleki usdt kanthi sukses.") {
                                this.log(`Ngoleki USDT kanthi sukses: ${usdtAmount}`, 'success');
                            }
                        }
                        ...
                        if (collectInfo.available_snitch > 0) {
                            ...
                            for (let j = 0; j < 3; j++) {
                                ...
                                if (snitchResult) {
                                    ...
                                    this.log(`Nyuwun Snitch kanthi sukses | wis ngumpulake ${snitchResult.available_snitch}/${collectInfo.snitch_amount}`, 'success');
                                }
